...
void setcolor(std::tuple<uint_fast8_t,uint_fast8_t,uint_fast8_t,uint_fast8_t> color_tuple) {
    setcolor(
            std::get<0>(color_tuple),
            std::get<1>(color_tuple),
            std::get<2>(color_tuple),
            std::get<3>(color_tuple)
            );
}
...
void draw_surface(const Surface& surface, float x, float y) {
    if (surface.getSurface() != NULL) {
        cairo_set_source_surface(   x11_state.ctx, surface.getSurface(),
                                    xworld_to_scrn(x) - cairo_image_surface_get_width(surface.getSurface())/2, 
                                    yworld_to_scrn(y) - cairo_image_surface_get_width(surface.getSurface())/2
                                );
